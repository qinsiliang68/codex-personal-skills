...
Windows PowerShell 5.1's filesystem provider can still fail on an existing path near or beyond the legacy `MAX_PATH` boundary. If `New-Item -ItemType HardLink` reports a missing partial path for a verified same-volume NTFS source, call `CreateHardLinkW` with absolute `\\?\`-prefixed source and destination paths; count files and bytes while linking instead of recursively re-enumerating the long-path staging tree. For authorized cleanup of that exact staging tree, first apply the containment checks in the destructive-operation boundary, then use `[IO.Directory]::Delete('\\?\' + $resolvedPath, $true)` in the same PowerShell process if `Remove-Item -Recurse` hits the same provider limit. Do not switch to `cmd.exe`, broaden the deletion target, or misclassify this provider failure as a missing source.
